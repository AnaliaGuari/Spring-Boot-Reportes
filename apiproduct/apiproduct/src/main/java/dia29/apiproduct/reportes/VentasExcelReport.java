package dia29.apiproduct.reportes;
...
public class VentasExcelReport {
    ...
    public VentasExcelReport(List<Bill> bills, List<BillDetails> billDetails)
    {
        this.bills = bills;
    ...
    private void readBody()
    {
        ...
        for(Bill b: bills)
        {
        	for(BillDetails bd: billDetails) {
        		if(bd.getBill().getIdBill() == b.getIdBill()) {
        			Row row = sheet.createRow(rC);
        			int cC = 0;
        			rC++;
        			createCell(row, cC++,b.getClients().getDni(), cS);
        			createCell(row, cC++,b.getClients().getName(), cS);
